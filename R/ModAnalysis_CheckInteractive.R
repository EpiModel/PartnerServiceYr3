...
df<-df.allsims %>%
  ...
  select(scenario, scenario1, sim, batch, time, year,
         ...
         #PS measures - All partners v partner testing
         found.partners.all,
...
#ART re-engagement
  ...
  #Table - ART reinit among Index-PP
    #gen.artreinit.elig<-getYrMeanTbl(df,gen.elig.for.reinit)
    gen.artreinit<-getYrMeanTbl(df,gen.reinit.tx)
    ...
    all.artreinit<-getYrMeanTbl(df,all.reinit.tx)
    
    # genreinit<-left_join(gen.artreinit.elig, gen.artreinit,by="scenario1")
    # ppreinit<-left_join(pp.artreinit.elig, pp.artreinit,by="scenario1")
    # partreinit<-left_join(part.artreinit.elig, part.artreinit,by="scenario1")
    allreinit<-left_join(gen.artreinit,pp.artreinit, by="scenario1");
    allreinit<-left_join(allreinit,part.artreinit, by="scenario1")
    allreinit<-left_join(allreinit,all.artreinit, by="scenario1")
    
    
    kable(allreinit,align = "lccc",booktabs=T, format="markdown",
          col.names = c("Scenario","General Pop.","Found PP","Found Partners","All")) %>%
      kable_classic() %>% kable_styling(latex_options="hold_position",position="center")
  
  #Plot
    reng<-df %>% 
      select(scenario1,time, pp.reinit.tx, gen.reinit.tx, part.reinit.tx) %>% 
      reshape2::melt(.,id.vars=c("scenario1","time"))
    
    #ART re-init among Gen, PP, Partners
    ggplot()+
      geom_smooth(data=reng, aes(x=time, y=value, group=variable, color=variable), se=F)+
      labs(x="Time (wks)",
           y="No. reengaged in care")+
      #scale_x_continuous(name="",expand = c(0.01,0.01))+
      theme_bw()+
      theme(panel.grid.major.y = element_line(colour = "grey76", size=0.1),
            panel.grid.minor=element_blank(),
            panel.grid.major.x=element_blank(),
            legend.title = element_blank())+
      facet_wrap(scenario1~.)
    
    #ART re-init among PP only
    ggplot()+
      geom_smooth(data=df, aes(time, pp.reinit.tx), se=F)+
      labs(x="Time (wks)",
           y="No. reengaged in care")+
      #scale_x_continuous(name="",expand = c(0.01,0.01))+
      theme_bw()+
